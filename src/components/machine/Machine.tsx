...
const Machine = ({ data }: MachineProps) => {
  return (
    <div className="relative bg-gray-800 rounded-lg p-6 w-64 h-96 shadow-2xl transform perspective-1000 rotate-x-6">
      ...
      <div className="relative z-10 bg-green-900 rounded-lg p-3 mb-4 h-32 font-mono text-green-300">
        <div className="flex justify-between text-sm">
          <span>Ventas: {data.CountSale}</span>
          <span>Crédito: ${data.CreditCost}</span>
        </div>
        <div className="text-center mt-2 text-xl">
          {data.TimeCycle}s
        </div>
        <div className="mt-2 text-xs flex justify-between">
          <span>Registros: {data.RecordCount}</span>
          <span>Límite: {data.CountLimit}</span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between mb-4">
        <div className="flex items-center">
          ...
          <span className="text-sm text-gray-300">{data.isEnable === '1' ? 'Habilitada' : 'Deshabilitada'}</span>
        </div>
      ...
      <div className="relative z-10 grid grid-cols-3 gap-2 mt-6">
        {[1, 2, 3, 4, 5, 6].map((btn) => (
          <button 
            key={btn}
            className="bg-gray-700 rounded-md p-2 text-gray-300 text-sm shadow-md hover:bg-gray-600 active:scale-95 transition-all"
          >
            {btn}
          </button>
        ))}
